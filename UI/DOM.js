...
var user = 'Adamskaya Yuliya';

var module = (function () {

        function functionForUser() {
            if (user === null) {
                var blockAvatar = document.getElementById("avatar");
        ...
        function addPhotoPost(objectPhotoPost) {
            ...
            newElem.innerHTML =
                `<div class="post">
<div class="image_name_date"><img class="post_image" src="images/avatar.png"/><p class="name">${objectPhotoPost.author}</p><p class="date_post">${objectPhotoPost.createdAt}</p></div></div>
<img class="photo" src="${ objectPhotoPost.photoLink}">
<div class="entry"><p class="description">${objectPhotoPost.description}</p><p class="hashTag">${objectPhotoPost.hashTag}</p><div class="like"><p>Like:</p><p>${objectPhotoPost.like}</p></div></div>
<button type="button" class="put_like"><img src="images/heart1.png" alt="like"/></button></div>`;
            if (objectPhotoPost.author === user) {
                var edit_delete = document.createElement("div");
                edit_delete.className = "delete_redact";
                edit_delete.innerHTML =
                    `<button type="button" class="delete"><img src="images/delete.png" alt="delete post"/></button>
<button type="button" class="redact"><img src="images/edit.png" alt="redact post"/></button>`;
